...
class User:
    ...
    def buy(self, seat, card):
        #Buy's a ticket if the card is valid
        if seat.is_free():
...
class Seat:
    ...
    def get_price(self):
        ...
        connecion = sqlite3.connect(self.database)
    ...
    def is_free(self):
        # Checks in database if a seat is taken or not
        connecion = sqlite3.connect(self.database)
        cursor = connecion.cursor()
        cursor.execute("""
    ...
    def occupy(self):
        # Change value of taken in database from 0 to 1 if Seat is free
        if self.is_free():
            connecion = sqlite3.connect(self.database)
            cursor = connecion.cursor()
            cursor.execute("""
...
class Ticket:
    ...
    def to_pdf(self):
        ...
        pdf.cell(w=0, h=100, txt="Your Digital Ticket", border=1, ln=1, align="C")
